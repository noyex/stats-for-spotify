...
import { getUserTopTracksShort } from '../services/spotifyService';
...
const TopSongs = () => {
  ...
  useEffect(() => {
    const fetchTopTracks = async () => {
      try {
        const data = await getUserTopTracksShort(userId);
        setTracks(data);
      } catch (err) {
      ...
      <div className="container">
        <div className="top-songs-header">
          ...
          <p className="top-songs-subtitle">
            Odkryj swoją ulubioną muzykę z ostatnich tygodni
          </p>
        </div>
        
        <div className="time-filter-tabs">
          <Link to={`/top-songs-short?id=${userId}`} className="time-filter-tab active">
            Ostatnie 4 tygodnie
          </Link>
